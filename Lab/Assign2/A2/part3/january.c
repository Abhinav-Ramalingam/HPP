...
{
    // Case 1: List is empty or item should be inserted at the front
    if (list == NULL || item.index < list->data.index)
    {
    ...
    // Case 2: Item with the same index exists, update min/max
    if (list->data.index == item.index)
    {
        list->data = item;  
        return list;
    }

    // Case 3: Recursive call to insert at the correct position
