...
void merge_sort(intType* list_to_sort, int N) {
  if(N == 1) {
    return;  // Base case: single element, no sorting needed
  }
  
  int n1 = N / 2;
  int n2 = N - n1;

  // Declare temporary arrays on the stack
  intType list1[n1];
  intType list2[n2];

  // Copy data to list1 and list2
  int i;
  for(i = 0; i < n1; i++) {
...
void modified_merge_sort(intType* list_to_sort, int N, int threshold) {
  if(N <= threshold) {
    bubble_sort(list_to_sort, N);  // Switch to bubble_sort for small lists
    return;
  }
  ...
  // Recursively sort the two sublists
  modified_merge_sort(list1, n1, threshold);
  modified_merge_sort(list2, n2, threshold);
