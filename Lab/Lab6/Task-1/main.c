#include <stdio.h>
#include <stdlib.h>
#include <sys/time.h>
#include "sort_funcs.h"

static double get_wall_seconds() {
  struct timeval tv;
  gettimeofday(&tv, NULL);
  double seconds = tv.tv_sec + (double)tv.tv_usec / 1000000;
  return seconds;
}

static int count_values(const int* list, int n, int x) {
  int count = 0;
  int i;
  for(i = 0; i < n; i++) {
    if(list[i] == x)
      count++;
  }
  return count;
}


int main(int argc, char* argv[]) {
  if(argc != 2) {
    printf("Please provide 2 arguments: N (number of elements to sort) and n (threshold for switching to bubble_sort).\n");
    return -1;
  }

  int N = atoi(argv[1]);  // Size of the list to sort
  int n = atoi(argv[2]);  // Threshold for switching to bubble_sort
  
  printf("N = %d, n = %d\n", N, n);

  if(N < 1) {
    printf("Error: (N < 1).\n");
    return -1;
  }

  // Allocate memory for the list to sort
  int* list_to_sort = (int*)malloc(N * sizeof(int));

  // Fill list with random numbers
  int i;
  for(i = 0; i < N; i++)
    list_to_sort[i] = rand() % 1000;

  // Count how many times the number 7 exists in the list
  int count7 = count_values(list_to_sort, N, 7);
  printf("Before sort: the number 7 occurs %d times in the list.\n", count7);

  // Sort the list using mb_sort
  double time1 = get_wall_seconds();
  single_merge_sort(list_to_sort, 0, N);  // Using mb_sort instead of merge_sort
  printf("Sorting list with length %d took %7.3f wall seconds.\n", N, get_wall_seconds() - time1);

  int count7_again = count_values(list_to_sort, N, 7);
  printf("After sort : the number 7 occurs %d times in the list.\n", count7_again);

  // Check that the list is really sorted
  for(i = 0; i < N - 1; i++) {
    if(list_to_sort[i] > list_to_sort[i + 1]) {
      printf("Error! List not sorted!\n");
      return -1;
    }
  }

  printf("OK, list is sorted!\n");

  free(list_to_sort);

  return 0;
}
