 ...
 **********************************************************************/
 ...
 {
     int rank, i;
     long start_index = (long)arg;
     int chunk_size = len / NUM_THREADS;
     int end_index = (start_index + 1) * chunk_size;
     if (start_index == NUM_THREADS - 1) {   
         end_index = len;
     }
     
     for (long j = start_index * chunk_size; j < end_index; j++) {
         rank = 0;
         for (i = 0; i < len; i++)
 ...
 int main(int argc, char *argv[]) {
     ...
     for (t = 0; t < NUM_THREADS; t++) {
         pthread_create(&threads[t], &attr, findrank, (void *)(long)t);
     }
