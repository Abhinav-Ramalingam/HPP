...
void *multiply_chunk(void *arg) {
  ...
  int end_row = (start_row + rows_per_thread < n) ? start_row + rows_per_thread : n;
...
int main(int argc, char *argv[]) {
  ...
  for (i = 0; i < NUM_THREADS; i++) {
    start_rows[i] = i * (n / NUM_THREADS);
    pthread_create(&threads[i], NULL, multiply_chunk, &start_rows[i]);
  }
